...
\begin{itemize}
    \item Develop a robust binary classification system capable of distinguishing between abstracts relevant and irrelevant to island ecosystem research
    \item Optimize the classifier for ranking performance rather than strict classification accuracy, recognizing that in practical information retrieval scenarios, the relative ordering of documents is more important than perfect binary classification
    \item Evaluate multiple state-of-the-art transformer architectures, with particular attention to domain-specific models pre-trained on biomedical literature
    \item Address the inherent class imbalance present in information retrieval datasets through appropriate loss functions and evaluation metrics
    \item Integrate the resulting classifier into the SIBiLS platform to assist researchers worldwide in efficiently identifying relevant biodiversity literature
\end{itemize}
...
BERT, or \textbf{Bidirectional Encoder Representations from Transformers}, represents a paradigm shift in how deep learning is applied to NLP \cite{devlin2019bert}. Instead of training a model from scratch for a specific task, BERT is a language representation model that is pre-trained on a massive amount of unlabeled text, and can then be quickly fine-tuned for various downstream tasks.
...
Hyperparameter optimization is essential for achieving optimal model performance and ensuring fair comparisons between different architectures. Rather than relying on default settings, we systematically search for the best combination of hyperparameters for each model and loss function combination.
...
The hyperparameter search space includes learning rates (1e-5 to 5e-5), batch sizes (8, 16, 32), warmup steps (0 to 10\% of total steps), and weight decay values (0.01 to 0.1). For models using Focal Loss, we also optimize the focusing parameter gamma (1.0 to 3.0) and the balancing factor alpha (0.1 to 0.5).
...
\section{Data Management and Experimental Design}
...
This query specifically targets recent environmental publications (2021-2025) while explicitly excluding island-related terms, ensuring that the synthetic negatives are genuinely irrelevant to the island biodiversity domain. The filter \texttt{fha[Filter]} restricts results to papers with available abstracts.
...
\begin{itemize}
    ...
    \item \textbf{Synthetic Negative Integration}: The 1,000 synthetic negatives retrieved from PubMed are combined with the original 449 manually curated documents. These synthetic negatives are labeled with -1 initially to distinguish them from original negatives (labeled 0) during the splitting process.
    
    \item \textbf{Stratified Cross-Validation Setup}: A 4-fold stratified cross-validation is implemented exclusively on the original 449 documents to ensure representative class distributions across folds. The stratification preserves the proportion of relevant vs. irrelevant documents in each fold.
    
    \item \textbf{Train/Dev/Test Splitting}: For each fold, the original data is split into training (≈56\%), development (≈19\%), and test (≈25\%) sets using stratified sampling. Crucially, all 1,000 synthetic negatives are added only to the training sets, ensuring that evaluation metrics are computed solely on manually curated data.
...
A significant characteristic of the dataset, after adding the optional negatives, is its imbalance. As shown in Figure \ref{fig:data_repartition}, the irrelevant class significantly outnumbers the relevant one. This is a common scenario in information retrieval and classification tasks, where the number of relevant documents is often a small fraction of the total corpus \cite{manning2008introduction,he2009learning}.
...
However, the current study's focus on island biodiversity research, combined with the specific curation criteria applied by domain experts at the University of Neuchâtel, provides some mitigation against this concern. The task requires understanding of domain-specific concepts and the ability to identify subtle indicators of relevance to island ecosystems, which extends beyond simple memorization \cite{thessen2012applications}.

Future work should consider establishing evaluation protocols that explicitly account for temporal splits, ensuring that test data comes exclusively from publications post-dating the pre-training corpora. This would provide a more conservative and realistic assessment of model performance in real-world deployment scenarios.
...
Our primary models are based on the Transformer architecture, leveraging pre-trained models from the Hugging Face library. The models investigated are the one introduced in \ref{tab:model_comparison}.
...
This chapter describes the software implementation of the BioMoQA classification pipeline, detailing the code structure and workflow from data preprocessing to final results generation.

\section{Code Architecture}

The implementation follows a modular architecture organized into three main components within the \texttt{src/} directory:

\subsection{Data Pipeline (\texttt{src/data\_pipeline/biomoqa/})}

The data processing pipeline consists of two main modules:

\begin{itemize}
    \item \textbf{\texttt{create\_raw.py}}: Handles the acquisition of synthetic negative examples from PubMed using the Biopython library. This module implements the PubMed query described in the Data section and processes the retrieved abstracts into a standardized format.
    
    \item \textbf{\texttt{preprocess\_biomoqa.py}}: Manages data cleaning, cross-validation fold generation, and train/dev/test splits. This module ensures stratified sampling, handles the integration of synthetic negatives with original data, and generates the fold structures used throughout the experimental pipeline.
\end{itemize}

\subsection{Model Training and Evaluation (\texttt{src/models/biomoqa/})}

The machine learning pipeline consists of several interconnected modules:

\begin{itemize}
    \item \textbf{\texttt{model\_init.py}}: Initializes transformer models from the Hugging Face library, configures tokenizers, and sets up classification heads with appropriate dropout layers.
    
    \item \textbf{\texttt{hpo.py}}: Implements hyperparameter optimization using Optuna's random search. This module handles the search space definition, trial execution, and best parameter selection based on ROC-AUC performance on development sets.
    
    \item \textbf{\texttt{train.py}}: Executes model training with the optimized hyperparameters. Implements early stopping, F1-score threshold optimization, and model checkpointing. Supports both Binary Cross-Entropy and Focal Loss training regimes.
    
    \item \textbf{\texttt{ensemble.py}}: Creates ensemble predictions by averaging the outputs of individual models trained within each fold. Handles both BCE and Focal Loss ensemble variants.
    
    \item \textbf{\texttt{baselines.py}}: Implements traditional machine learning baselines including SVM with TF-IDF features for comparison with transformer models.
\end{itemize}

\subsection{Orchestration and Configuration}

The experimental workflow is orchestrated through shell scripts located in the \texttt{scripts/} directory:

\begin{itemize}
    \item \textbf{\texttt{launch\_final.sh}}: The main execution script that coordinates the entire pipeline from data preprocessing through ensemble creation. This script iterates over all model-loss combinations, manages cross-validation folds, and ensures proper sequencing of HPO and training phases.
    
    \item \textbf{\texttt{config.py}}: Centralizes all configuration parameters including random seeds, file paths, and experimental settings to ensure consistency across all pipeline components.
\end{itemize}

\section{Experimental Workflow}

The implementation follows the workflow illustrated in Figure \ref{fig:workflow}:

\begin{enumerate}
    \item \textbf{Data Preparation}: Raw data is processed and augmented with synthetic negatives, then split into 4 cross-validation folds with stratified sampling.
    
    \item \textbf{Model Training}: For each fold and each model-loss combination, hyperparameter optimization is performed followed by final model training with the best parameters.
    
    \item \textbf{Evaluation}: Models are evaluated on their respective test folds, with metrics computed and saved to CSV files in the \texttt{results/biomoqa/} directory.
    
    \item \textbf{Ensemble Creation}: Individual model predictions are combined to create ensemble models for each loss function and fold combination.
\end{enumerate}
...
\midrule
\multirow{5}{*}{BCE} & bert-base-uncased & 0.717 & 0.859 & 0.831 & 0.889 & 0.348 \\
...
\midrule
\multirow{5}{*}{Focal} & bert-base-uncased & 0.648 & 0.848 & 0.747 & 0.986 & 0.016 \\
...
The comparison between BCE and Focal Loss reveals interesting patterns in how these loss functions handle class imbalance. As shown in Table \ref{tab:main_results}, Focal Loss models exhibit markedly different behavior compared to BCE models:
...
\subsection{Dataset Size and Generalizability}
...
\subsection{Temporal Evaluation Bias}
...
\subsection{Statistical Testing Assumptions}
...
This violation of the independence assumption could lead to inflated confidence in the statistical significance of observed differences between models. A more rigorous approach would require techniques specifically designed for dependent samples or larger-scale experiments with truly independent datasets. Nevertheless, the substantial effect sizes observed between model categories (e.g., domain-specific vs. general-purpose) suggest that the main conclusions remain valid despite this limitation.
...
The broader impact of this work extends beyond the specific domain of island biodiversity, providing insights into transformer-based classification for scientific literature and demonstrating the practical challenges and solutions for deploying AI systems in specialized scientific domains \cite{cohen2010overview,westergaard2018comprehensive}. As part of the open science initiative, the resulting system will contribute to making scientific knowledge more accessible and discoverable for the global research community.
