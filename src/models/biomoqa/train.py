...
def train(cfg,hp_cfg):
    ...
    config = get_config()
    train_split = load_dataset("csv", data_files=str(config.get_fold_path("train", cfg['fold'], cfg['run'])), split="train")
    dev_split = load_dataset("csv", data_files=str(config.get_fold_path("dev", cfg['fold'], cfg['run'])), split="train")
